...
class Agent:
    ...
    Attributes:
        ...
        _cv_engine (CVEngine): The agent CV engine
    ...
    def receive_image(self, session_id: str, img: np.ndarray):
        ...
        Args:
            session_id (str): the session that sends the message to the agent
            img (np.ndarray): the image sent to the agent
        """
        session = self._sessions[session_id]
        # TODO: Raise exception SessionNotFound
    ...
    def use_websocket_platform(self, use_ui: bool = True, video_input: bool = False) -> WebSocketPlatform:
        ...
        Args:
            ...
            video_input (bool): if true, the WebSocket client will send streaming video (images) to the agent
